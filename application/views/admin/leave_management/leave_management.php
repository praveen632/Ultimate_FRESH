...
<td><select onchange="status_update(this.value,'<?php echo $v_all_leave->leave_application_id;?>','<?php echo $v_all_leave->user_id ?>');">
  <option value="1"<?php if($v_all_leave->application_status==1){ echo 'selected';}?>>Pending</option>
  <option value="2"<?php if($v_all_leave->application_status==2){ echo 'selected';}?>>Accepted</option>
  <option value="3"<?php if($v_all_leave->application_status==3){ echo 'selected';}?>>Rejected</option>
...
<script>
    ...
    function status_update(status,id,approve_by)
    {
        $.ajax({
            type: "post",
            url: "<?php echo base_url(); ?>admin/leave_management/change_status_admin",
            data:{'status':status,'id':id,'approve_by':approve_by},
            success: function(data) 
            {
              window.location.href = '<?php echo base_url();?>admin/leave_management';                                
            }
          });
    }
</script>