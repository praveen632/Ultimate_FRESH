...
class Leave_Management extends Admin_Controller
{
    ...
    public function save_leave_application()
    {
        ...
        if (strtotime($data['leave_start_date']) > strtotime($data['leave_end_date'])) {
            $type = "error";
    ...
    public function change_status_admin(){
        $data['application_status'] = $_REQUEST['status'];
        $data['approve_by'] = $_REQUEST['approve_by'];
        $ids = $_REQUEST['id'];
        $where = array('leave_application_id' => $ids);
        $data = $this->application_model->set_action($where, $data, 'tbl_leave_application');
        return $data;
    }
