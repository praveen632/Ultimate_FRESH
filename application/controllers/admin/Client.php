...
class Client extends Admin_Controller
{
    ...
    public function save_client($id = NULL)
    {
        ...
        if (!empty($id)) {
            ...
            $password = strlen($this->db->where($where)->get('tbl_client')->row()->password);
...
//            'URL' => site_url('-') . '/' . $c_slug . '/',
        ...
        if (!empty($client_contacts)) {
            foreach ($client_contacts as $contact) {
                ...
                $c_path = set_realpath('-/' . $c_slug . '/' . $contact->media_path_slug);
                ...
                $root_options['URL'] = site_url('-/' . $contact->media_path_slug) . '/';
