...
class Settings extends Admin_Controller
{
    ...
    public function save_system()
    {
        $input_data = $this->settings_model->array_from_post(array('default_language', 'locale',
            'timezone', 'default_currency', 'default_account', 'date_format',
            'enable_languages', 'allow_client_registration', 'allow_client_project', 'currency_position', 'money_format', 'allowed_files', 'google_api_key',
    ...
    public function update_email()
    {
        ...
        if (!empty($smtp_pass)) {
            $password = strlen(config_item('smtp_pass'));
            $old = str_repeat('*', $password);
            if ($old != $smtp_pass) {
                $data = array('value' => $smtp_pass);
            } else {
                $data = array('value' => config_item('smtp_pass'));
            }
    ...
    public function sent_test_email()
    {
        ...
        if (!empty($test_email)) {
            ...
            $result = $this->settings_model->send_email($params);
            if ($result == true) {
...
//            }
    ...
    public function save_theme()
    {
        $input_data = $this->settings_model->array_from_post(array('website_name', 'logo_or_icon', 'sidebar_theme', 'aside-float', 'aside-collapsed', 'layout-boxed', 'layout-fixed', 'login_position', 'RTL',
            ...
            'active_background', 'active_color', 'body_background'));
    ...
    public function save_email_integration()
    {
        ...
        $password = strlen(config_item('config_password'));
        $old = str_repeat('*', $password);
        if ($old != $input_data['config_password']) {
            $input_data['config_password'] = $input_data['config_password'];
        } else {
            $input_data['config_password'] = config_item('config_password');
        }
