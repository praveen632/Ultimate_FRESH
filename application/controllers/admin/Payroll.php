        ...
        if (!empty($notifyUser)) {
            foreach ($notifyUser as $v_user) {
                add_notification(array(
                    ...
                    'icon' => 'cc-mastercard',
                    'link' => 'admin/payroll/view_advance_salary/' . $salary_info->advance_salary_id,
                    'value' => display_money($salary_info->advance_amount, $curency->symbol),
                ));
            }
        }
        if (!empty($notifyUser)) {
...
//        // save into tbl employee paymenet
        ...
        if (!empty($employee_info->hourly_rate_id) || !empty($employee_info->salary_template_id)) {
            ...
            $deduct_from_default_account = $this->input->post('deduct_from_default_account', true);
            if (!empty($deduct_from_default_account)) {
