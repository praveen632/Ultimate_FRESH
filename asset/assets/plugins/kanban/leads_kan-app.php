<script type="text/javascript">
    ...
    (function () {
        ...
        function leads_kanban_init() {
            ...
            ?>
            addList('<?= $v_leads_status->lead_status . ' ' . $lead_type ?><span><?= count($k_all_leads)?></span>',<?= $v_leads_status->lead_status_id?>);
            ...
            $text = null;
            $text .= '<a data-lead-status-id="' . $v_leads_status->lead_status_id . '" data-leads-id="' . $v_k_leads->leads_id . '" style="word-wrap: break-word;" class="text-info" href="' . base_url() . 'admin/leads/leads_details/' . $v_k_leads->leads_id . '">' . $v_k_leads->lead_name . '</a>';
            $text .= '<div class="">';
            $text .= '<strong class="text-sm  m0 p0 ">' . lang('source') . ': <span class="text-danger">' . $k_lead_source->lead_source . '</span></strong>';
